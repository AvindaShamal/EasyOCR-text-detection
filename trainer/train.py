...
def train(opt, show_number=2, amp=False):
    ...
    with tqdm(
        total=opt.num_iter, desc="Training Progress", unit="iter", initial=i
    ) as pbar:
...
def plot_learning_curves(opt, metrics: list) -> None:
    """Plots the learning curves for the model.
    Args:
        metrics : A list containing the training loss and accuracy for each epoch.
    Returns:
        None
    """
    ...
    plt.legend()
    plt.savefig(f"./saved_models/{opt.experiment_name}/loss.png")
    ...
    plt.legend()
    plt.savefig(f"./saved_models/{opt.experiment_name}/accuracy.png")
...
def main():
    ...
    train(opt, amp=False)
    plot_learning_curves(opt, metrics)
